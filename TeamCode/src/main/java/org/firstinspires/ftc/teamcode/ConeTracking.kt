...
class RedConeTracking: LinearOpMode(){
    ...
    inner class ConePipeline: OpenCvPipeline(){
        ...
        override fun processFrame(input: Mat?): Mat? {           //cap.read(frame);
            ...
            val lower_bound = Scalar(154.0, 198.0, 0.0)
            val upper_bound = Scalar(179.0, 255.0, 255.0)
...
class BlueConeTracking: LinearOpMode(){
    ...
    inner class ConePipeline: OpenCvPipeline(){
        ...
        override fun processFrame(input: Mat?): Mat? {           //cap.read(frame);
            ...
            val lower_bound = Scalar(110.0, 50.0, 50.0)
            val upper_bound = Scalar(130.0, 255.0, 255.0)
